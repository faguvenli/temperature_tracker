...
@section('title') Çanta Cihazları @endsection
    ...
    @if(auth()->user()->isSuperAdmin)
        <select id="region" class="form-select select2">
            ...
            @foreach($regions as $region)
                <option value="{{ $region->id }}" {{ (session()->get('bag_device_region') == $region->id) ? 'selected' : null }}>
                {{$region->name}}
                </option>
            @endforeach
        </select>
...
<script>
    $(document).on("change", "#region", function(e) {
        axios.post('{{route('bag-device-setRegion')}}', {region:$(this).val()})
            .then(() => {
